            ...
            <table class="table mt-5">
                <thead class="border-b-2 border-solid border-black text-left">
                    <tr>
                        ...
                        <th scope="col" class="py-2 pl-3">{{ __('task_status.name') }}</th>
                        <th scope="col" class="py-2 pl-3">{{ __('task_status.created_by_id') }}</th>
                        <th scope="col" class="py-2 pl-3">{{ __('task_status.created_at') }}</th>
                        @auth
                            <th scope="col" class="py-2 pl-6">{{ __('task_status.actions') }}</th>
                        @endauth
                    </tr>
                </thead>
                <tbody>
                    @foreach($statuses as $status)
                        <tr class="border-b border-black text-left">
                            ...
                            <td class="py-2 pl-3">{{ $status->name }}</td>
                            <td class="py-2 pl-3">{{ $status->createdBy->name }}</td>
                            <td class="py-2 pl-3">{{ $status->created_at->format('d.m.Y') }}</td>
                            <td class="py-2 pl-6">
                                @can('delete', $status)
                                ...
                                @can('update', $status)
                                    {{ html()->a(route('task_statuses.edit', $status), __('task_status.edit'))
                                        ->class('btn btn-sm btn-outline-primary text-blue-600 hover:text-blue-900') }}
                                @endcan
                            </td>
                        </tr>
                    @endforeach
